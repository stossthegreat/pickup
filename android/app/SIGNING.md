...
## Fingerprints (registered with Play Console)
...
Source of truth: `android/app/play-fingerprints.properties`. The
gradle build reads it and exposes the values as
`BuildConfig.PLAY_UPLOAD_KEY_SHA1` / `PLAY_UPLOAD_KEY_SHA256`, so the
AAB carries them as embedded constants.

The CI workflow's `Verify keystore` step prints the keystore's live
SHA-1 to the build log so you can compare against the value above
