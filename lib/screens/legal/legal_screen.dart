...
LegalDoc get termsDoc => LegalDoc(
  title: 'Terms of Use',
  subtitle: 'FIRSTMOVE · THE AGREEMENT',
  ...
  sections: [
    ...
    LegalSection('FIRSTMOVE PRO — WHAT YOU GET, PRICE & AUTO-RENEWAL',
      'FirstMove Pro is a single auto-renewing weekly subscription. This is '
      ...
      'PRICE & BILLING\n\n'
      '• FirstMove Pro — \$6.99 USD per week (or the local-currency '
    ...
    const LegalSection('ABOUT MIRRORLY',
      'FirstMove is a self-assessment and self-training tool for '
      ...
      'FirstMove is not a medical device and does not provide '
      'medical, dental, psychological, or surgical advice.'),
    const LegalSection('WHO CAN USE THIS APP',
      'You must be at least 13 years old to use FirstMove. If you are '
    ...
    LegalSection('ACCOUNTS',
      'FirstMove does not require an account. Purchases and saved '
    ...
    LegalSection('SUBSCRIPTIONS & AUTO-RENEWAL',
      'FirstMove offers an auto-renewing subscription:\n\n'
      '• FirstMove Pro Weekly — \$6.99 USD per week (or local '
      ...
      // '• FirstMove Pro Annual — \$139.99 USD per year (or local '
      // 'equivalent), billed annually until cancelled.\n'
      '\n'
      'This subscription unlocks every FirstMove Pro feature — scans, '
      ...
      '$_platformAppleStore directly, not by FirstMove.'),
    const LegalSection('ONE-TIME CREDIT PACKS',
      'Credit packs are non-subscription, one-time purchases. '
      'FirstMove Rescue Pack — \$9.99 USD (or local equivalent) — '
    ...
    const LegalSection('WHAT WE RENDER — AND WHAT WE DO NOT',
      'FirstMove renders illustrative previews of grooming and '
      ...
      'barber, surgeon, or dermatologist. Never use an FirstMove '
    ...
    const LegalSection('YOUR CONTENT',
      'You retain all rights to photos you take inside FirstMove. By '
      'scanning a photo and granting in-app permission in the AI data '
      'consent dialog, you grant FirstMove a limited, revocable, '
    ...
    const LegalSection('AI DATA PERMISSION — EVERY DETAIL OF WHAT GETS SENT, WHERE, AND WHY',
      ...
      'device, FirstMove displays a full-screen permission dialog '
      ...
      '+ cdingram/face-swap.\n'
      '• FirstMove\'s own backend on Railway — transient routing '
      'only.\n\n'
      ...
      '• On FirstMove\'s backend: bytes not persisted; only '
    ...
    const LegalSection('VOICE & TRAINING DATA — EYES AND GAME TABS',
      ...
      'Phone → FirstMove\'s voice backend at '
      ...
      'FirstMove\'s voice backend; only timestamps and HTTP status codes are '
      ...
      'token from FirstMove\'s voice backend (HTTPS) → phone opens a TLS-'
      ...
      'synthesis.\n'
      '• FirstMove\'s voice backend on Railway — transient routing '
      ...
      '• In flight: TLS 1.3 encrypted.\n'
      '• FirstMove\'s voice backend: bytes not persisted; only HTTP '
    ...
    const LegalSection('FACE DATA — WHAT WE COLLECT, WHY, WHO RECEIVES IT, HOW LONG WE KEEP IT',
      ...
      'FirstMove collects two related pieces of face data:\n'
      ...
      'FirstMove does NOT use face data for: facial recognition, '
      ...
      '• On FirstMove servers — request timestamps and status codes '
      ...
      'On FirstMove\'s servers: never retained.\n'
    ...
    const LegalSection('THIRD-PARTY PROTECTION PARITY',
      ...
      'receives FirstMove user data must provide the same or equal '
      'privacy protection as FirstMove itself. Both providers we '
      ...
      'FirstMove does not transmit user photos to any other third '
    ...
    const LegalSection('ACCEPTABLE USE',
      'You agree not to use FirstMove to scan, analyse, or render a '
      ...
      'that you have no right to share. You agree not to use FirstMove '
    ...
    const LegalSection('DISCLAIMERS & LIABILITY',
      'FirstMove is provided "as is" without warranty of any kind, '
      'express or implied. To the maximum extent permitted by law, '
      'FirstMove\'s total liability for any claim is limited to the '
      'amount you paid FirstMove in the twelve months preceding the '
      'claim.'),
...
LegalDoc get privacyDoc => LegalDoc(
  ...
  sections: [
    ...
    const LegalSection('FACE DATA — QUICK SUMMARY',
      ...
      'call, then dropped. Neither FirstMove nor the third parties we '
      ...
      '• On FirstMove\'s own backend: never persisted. The backend acts '
    ...
    const LegalSection('AI DATA PERMISSION — EVERY DETAIL',
      ...
      'social-login data.\n\n'
      'EXACT ROUTE\n\n'
      'Phone → FirstMove backend (mirrorly-production.up.railway'
      ...
      '• FirstMove\'s own backend (Railway), transient routing.\n'
      'No other party.\n\n'
      'RETENTION\n\n'
      '• On your phone: until you delete or uninstall.\n'
      '• In flight: TLS 1.3 encrypted.\n'
      '• FirstMove backend: bytes not persisted; only timestamps '
    ...
    const LegalSection('VOICE & TRAINING DATA — EYES AND GAME TABS',
      ...
      'EXACT ROUTE\n\n'
      'Recorded drills: phone → FirstMove\'s voice backend (https://'
      'auralayai-production-65c2.up.railway.app) over HTTPS / TLS '
      '1.3 → backend forwards to OpenAI in-memory for one request '
      '→ response returns. FirstMove\'s voice backend does NOT persist '
      ...
      'OpenAI Realtime API token from FirstMove\'s voice backend (HTTPS) '
      '→ phone opens a TLS WebSocket directly to api.openai.com → '
      'live audio streams to OpenAI; replies stream back. Audio '
      'never traverses FirstMove servers in this mode.\n\n'
      'OPENAI MODELS\n\n'
      ...
      'not stored.\n'
      '• In flight: TLS 1.3 encrypted.\n'
      '• FirstMove\'s voice backend: bytes not persisted; only status + '
    ...
    const LegalSection('RIZZ — SCREENSHOTS, CHAT & PICKUP LINES',
      ...
      'EXACT ROUTE\n\n'
      'Phone → FirstMove backend (mirrorly-production.up.railway.app) over '
      ...
      '• FirstMove\'s own backend (Railway), transient routing only.\n'
    ...
    const LegalSection('FACE DATA — WHAT IT IS, WHAT IT ISN\'T',
      'FirstMove uses on-device computer vision to derive geometric '
      ...
      'FirstMove server.\n'
      '• No model training. Neither FirstMove nor any third party we '
    ...
    const LegalSection('THIRD-PARTY PROTECTION PARITY',
      'Per App Store guideline 5.1.2(i), any third party that '
      'receives FirstMove user data must provide the same or equal '
      'privacy protection as FirstMove itself. Both AI providers '
      'meet this bar:\n\n'
      '• OpenAI — under the standard API terms, customer inputs are '
    ...
    const LegalSection('CHILDREN',
      'FirstMove is not intended for children under 13. We do not '
    ...
    LegalSection('PURCHASES',
      ...
      '($_storeName). FirstMove never sees your card number. We see '
