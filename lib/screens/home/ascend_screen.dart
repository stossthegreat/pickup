...
///      Dangerous → Magnetic → FirstMove. Status ladder, not stats.
...
class _AscendScreenState extends State<AscendScreen> {
  ...
  /// FIRSTMOVE score and the prior weekly snapshot. Pre-loaded on first
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            // ── 2 — FIRSTMOVE SCORE. The composite number that unifies
  ...
  _ScanMilestone? _scanMilestone(int day) {
    ...
    if (day >= 56 && day <= 60) {
      return const _ScanMilestone(
        ...
        subtitle: 'The Day-60 scan unlocks the FIRSTMOVE CERTIFIED card. '
  ...
  /// v291 — Generate the FIRSTMOVE CERTIFIED Day-60 share card.
  /// Collects:
  ///   - BEFORE photo: first scan in history (chronological)
  ///   - AFTER photo:  last scan in history (the Day-60-window scan)
  ///   - FIRSTMOVE SCORE arc: composite computed at Day-1 conditions
  ...
  Future<void> _generateCertificate() async {
    ...
    // FIRSTMOVE SCORE arc — same formula AscensionService runs in the
...
/// v289 — FIRSTMOVE SCORE hero. The composite that levels the whole
...
class _FinalFormCard extends StatelessWidget {
  ...
  /// (first/last scan, looks/game arcs, FIRSTMOVE start/end) and the
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              unlocked
                ? 'You finished the protocol. Generate the receipt — '
                  'real before / after photos, the FIRSTMOVE SCORE arc, '
            ...
            for (final line in const [
              ...
              'FIRSTMOVE SCORE arc — start to Day 60',
